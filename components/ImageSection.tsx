...
import { Image as ImageIcon, Sparkles, Download, Loader2, Maximize2, Share2 } from 'lucide-react';
      ...
      <div className="p-8 md:p-12 border-b border-white/5">
        <div className="max-w-5xl mx-auto space-y-8">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-3xl font-bold text-white tracking-tight">Estúdio de Design</h1>
              </div>
              <p className="text-slate-400 text-sm">Crie conceitos visuais de alta qualidade em segundos.</p>
            </div>
          ...
          <div className="relative group">
            <div className="absolute -inset-1 bg-gradient-to-r from-pink-500 to-rose-500 rounded-3xl blur opacity-20 group-focus-within:opacity-40 transition duration-500"></div>
            <div className="relative glass rounded-3xl p-2 flex flex-col md:flex-row gap-2 border-white/10">
              <textarea
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Ex: Uma sala de estar minimalista com iluminação inteligente e vista para as montanhas ao pôr do sol..."
              ...
              <button 
                ...
                className={`px-8 py-4 rounded-2xl transition-all flex items-center justify-center gap-3 ${
                  ...
                  : 'bg-white text-slate-900 hover:bg-pink-50 hover:scale-105 active:scale-95 shadow-xl font-bold'
                }`}
              >
                {isGenerating ? (
      ...
      <div className="flex-1 overflow-y-auto p-8 md:p-12 no-scrollbar">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 gap-6 space-y-6">
            {assets.map((asset) => (
              <div key={asset.id} className="group relative glass rounded-3xl overflow-hidden border border-white/10 hover:border-pink-500/30 transition-all duration-500 break-inside-avoid shadow-2xl">
                <img 
                  src={asset.url} 
                  alt={asset.prompt} 
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950 via-slate-950/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-6">
                  ...
                  <div className="flex gap-2">
                    <a 
                      href={asset.url}
                      download={`casar-${asset.id}.png`}
                      className="flex-1 bg-white/10 hover:bg-white/20 backdrop-blur-xl border border-white/10 rounded-xl py-2.5 flex items-center justify-center gap-2 text-xs font-bold text-white transition-all active:scale-95"
                    >
            ...
            {assets.length === 0 && !isGenerating && (
              <div className="py-32 text-center col-span-full">
                ...
                <h3 className="text-xl font-bold text-slate-400">Nenhuma criação ainda</h3>
                <p className="text-slate-500 mt-2 max-w-xs mx-auto">Descreva seu conceito acima para começar a materializar suas ideias.</p>
              </div>
            )}

            {isGenerating && (
              <div className="glass rounded-3xl aspect-[4/5] flex flex-col items-center justify-center gap-6 animate-pulse border-pink-500/20 shadow-2xl shadow-pink-500/5">
                ...
                <div className="space-y-2 text-center">
                  <p className="text-sm font-bold text-pink-400 uppercase tracking-widest">Materializando</p>
                  <p className="text-xs text-slate-500">Isso levará apenas alguns segundos...</p>
                </div>
              </div>
            )}
          </div>
